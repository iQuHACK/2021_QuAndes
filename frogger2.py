...
#intizializing the game
    def init(self):
        ...
        self.running = True
        self.state = 'START'
        self.bot=QAOA_BOT(direcciones,matrix)
        #Player's frog 
        ...
        self.lanes.append( Lane( 2, t='log', c=(153, 217, 234), n=2, l=4, spc=350, spd=0.03) )
        ...
        self.lanes.append( Lane( 5, t='log', c=(153, 217, 234), n=2, l=2, spc=230, spd=-0.02) )
        ...
        self.lanes.append( Lane( 11, t='car', c=(195, 195, 195), n=2, l=4, spc=200, spd=0.05) )
...
#Method that describes the surroundings of the bot and creates a matrix according to it, this matrix will help the bot to choose the best path possible, it is the input to an implemeted qaoa
    def fillmatrix(self):
        ...
        global matrix
        pos = [-1,0,1]
        newMatrix=np.array([[-1,-1,-1],[-1,-1,-1],[-1,-1,-1]])
        for j in pos:
            for i in pos:
                ...
                if  lane_index !=12 and lane_index!=11 and lane_index !=0 and lane_index !=1 and lane_index!=13:
                    if self.lanes[lane_index].obstacles ==[]:
                        ...
                        for obstacle in self.lanes[lane_index].obstacles:
                            ...
                            oright,
                            otop,
                            obottom, "compare:",left,
                            right,
                            top,
                            bottom)
                            if not (int(left) >= oright or int(right) <= oleft or int(top) >= obottom or int(bottom) <= otop):
                                print (i+1,j+1)
                                newMatrix[j+1][i+1] = 1
    ...
    def execute(self):
        ...
        while self.running:
            for event in pygame.event.get():
                self.event( event )
            self.fillmatrix()
            self.bot.direcciones=direcciones
            self.bot.vision=matrix
